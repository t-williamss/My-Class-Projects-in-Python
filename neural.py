# Rett Bull - a simple backpropagation program for experiments in EECS 295.
import math, random
from typing import List, Tuple, Any, TypeVar

# Arrays are represented as lists of (equal-sized) lists. Packages like numpy
# could speed up computation if needed
...
def make_random_array(m: int, n: int, lower: float = -2.0, upper: float = 2.0) -> List[List[float]]:
...
# Two activation functions, sigmoid & tanh, and their derivatives. Activation
# functions take the raw output of a node and essentially determine if it fires.
# They often tend to compress the range of output in some way for cleaner
# gradients and input to subsequent layers.

def sigmoid(x: float) -> float:
	"""Computes 1/(1+e^-x)

	Args:
		x - the value to apply the sigmoid too (in our neural net corresponds to
...
def tanh(x: float) -> float:
	...
	Args:
		x - the value to apply the hyperbolic tangent too (in our neural net
			corresponds to the output of a single node)
...
class NeuralNet:
	"""A simple implementation of a neural net. Has three layers - input, output
	and one hidden. Contains three lists to hold the activation values for each of
	the layers and four arrays (two for the weights and two more
	...
	Attributes:
		...
		ih_weights - matrix of weights from input layer to hidden layer (represented
			as nested arrays) Each inner array holds weights mapping all input layer
			nodes to one hidden layer node
		ho_weights - matrix of weights from hidden layer to output layer
			(represented as nested arrays) Each inner array holds weights mapping all
			input layer nodes to one hidden layer node
		...
		dact_function - derivate of activation function (defaults to d_sigmoid),
			must match the activation function
	"""
	...
	def evaluate(self, inputs: List[Any]) -> List[float]:
		...
		if len(inputs) != (self.num_input - 1):
			raise self.SizeMismatch(self.num_input - 1, len(inputs))

		# set input layer + added bias
		self.input_layer = inputs + [1.0]

		# compute input -> hidden activations
		self.hidden_layer = self.compute_one_layer(self.input_layer, self.num_hidden,
			self.ih_weights, True)
		# compute hidden -> output activations
		self.output_layer = self.compute_one_layer(self.hidden_layer, self.num_output,
			self.ho_weights, False)

		# Return a copy of the output layer.
		return self.output_layer[:]

	def compute_one_layer(self, curr_layer: List[float], num_next_layer: int,
			weights: List[List[float]], is_hidden_layer: bool) -> List[float]:
		"""Compute one step of forward propagation (calculate activations of layer
		x+1 given activations of layer x and weights from x -> x+1)
	...
	I = List[Any]
	O = List[Any]
	def test(self, data: List[Tuple[I, O]]) -> List[Tuple[I, O, O]]:
		...
		Python typing is still relatively new and has issues with restraining types
		to int or float). The return type is a list of (input, expected output,
		actual output) triples.
		...
		Returns:
			...
			output are the passed in lists while actual output is a list of the
			neural net's output
		"""
		return [(_in, expected, self.evaluate(_in)) for _in, expected in data]

	def train(self, data: List[Tuple[I, O]], learning_rate: float = 0.5,
		momentum_factor: float = 0.1, iters: int = 1000, print_interval: int = 100) -> None:
		...
		for i in range(print_count):
			for j in range(print_interval-1):
				one_pass()
			print("error %-14f" % one_pass_with_error())

		# finish any remaining passes
		for i in range(left_over):
			one_pass()

	def back_propagate(self, inputs: List[Any], desired_result: List[Any],
		learning_rate: float, momentum_factor: float):
		"""The algorithm for adjusting weights

		Computes influence of each node based on derivatives to determine how to
		adjust weights.
		...
		output_deltas = [self.dact_function(out) * (des - out) for out, des in
			zip(outputs, desired_result)]
		...
		for h in range(self.num_hidden):
			...
			self.ho_weights[h][o] += (learning_rate * change +
				momentum_factor * self.ho_weights_changes[h][o])
		...
		for i in range(self.num_input):
			for h in range(self.num_hidden - 1):
				...
				self.ih_weights[i][h] += (learning_rate * change +
	...
	class SizeMismatch(Exception):
		"""A class to represent an error when the wrong number of input values is
		offered
